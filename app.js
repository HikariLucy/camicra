...
function initEventoManualForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const titulo = document.getElementById('tituloEventoManual').value.trim();
        const fecha = document.getElementById('fechaEventoManual').value;
        const desc = document.getElementById('descEventoManual').value.trim();
        ...
        try {
            ...
            if (!response.ok) throw new Error('Error al guardar el evento en el servidor');

            alert('¡Evento añadido con éxito al calendario!');
            form.reset();
